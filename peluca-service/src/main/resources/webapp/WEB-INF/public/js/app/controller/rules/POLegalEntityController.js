 ...
 */
POLegalEntityController = (function () {
	var firstAdd = true;
	var showAddUpdateCombos = function(){
        $("#spProductType").focus();
    ...
    var init = function () {
        ...
        $('#modalAddRule').on('show.bs.modal', function (e) {
        	...
        	if (firstAdd){
        		firstAdd = false;
        		initProductType(null,[$("#spProductType"),$("#spProductTypeUnique")],false, true, true);
                initSellType(null,$("#spSellType"),true, true);
                initVCC(null,$("#spVCC"));
                initCountries(null, [$('#spCountrySite'),$('#spCountrySiteUnique'),$('#spCountryDestiny'),$('#spCountryDestinyUnique')], null, null, showAddUpdateCombos);
        	}else{
        		showAddUpdateCombos();
        	}
        });
        ...
        $("#btnSpRemoveAll").on("click", function () {
            ...
            if (list.length > 0) {
                ButtonRemove.remove(list,"po-legal-entity","#tSpResult","rowid",getLastVersion);
            } else {
        ...
        get();
        LegalEntityService.get(legalEntityHandler);
        initCountries(null, [$('#filterSpCountrySite'),$('#filterSpCountryDestiny')],null, true);
        initProductType(null,[$("#filterSpProductType")],true, true, false );
        initSellType(null,$("#filterSpSellType"),true, true);
        initModel();
        initVCC(null,$("#filterSpVCC"));
        initToolTips();
    ...
    var initCountries = function(params, listElement,firstOption, blankOptionEnabled, toApply){
    ...
    var initSellType = function(params,element,firstOption, blankOptionEnabled){
        CatalogueService.getSellType(function(list){
            ValuesService.buildCombo(element, list, firstOption, blankOptionEnabled);
        },params);
    };

    var initModel = function(params){
    ...
    var initVCC = function(params, element){
        CatalogueService.getVCC(function(list){
            ValuesService.buildCombo(element, list, true, true);
        },params);
    };

    var initProductType = function (params,listElement,emptyOption, enableFirstOpt, applyMultiselect) {
        CatalogueService.getProductType(function (list) {
	   	$.each(listElement, function( key, value ) {
 			ValuesService.buildCombo(value, list, emptyOption, enableFirstOpt);
 			if (applyMultiselect){
 				ValuesService.applyMultiselect(value);
 			}
	 	});
        },params);
    };
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        ...
                        return "<span><a id='rowRemoveBtn_" + row.id + "' href='javascript:void(0);' onclick='javascript:POLegalEntityController.removeOnly(\"" + idList + "\");' title='Remover fila'><i class='glyphicon glyphicon-trash'></i></a></span>";
                    }
                }
            ],
            "language": {
    ...
    var removeOnly = function (id) {
        ButtonRemove.removeOnly(id,"po-legal-entity","#tSpResult","rowid",getLastVersion);
    };

    var legalEntityHandler = function (data) {
    ...
    var countryHandler = function (countryList) {
    ...
    var relaseNewVersion = function (versionDate) {
        var settings = new Object();
