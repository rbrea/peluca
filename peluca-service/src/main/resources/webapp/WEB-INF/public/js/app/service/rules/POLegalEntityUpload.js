POLegalEntityUpload = function () {
}

POLegalEntityUpload.init = function () {
    ...
    $('#modalImport').on('hidden.bs.modal', function () {
        ...
        POLegalEntityController.reset();
        $("#btnSpSearch").click();
    })

    $('#modalImport').on('shown.bs.modal', function () {
        $("#ruleSellPointResultTable").DataTable().destroy();
        RestConnector.PostFile('#ruleSellPointfileupload', '/app/service/po-legal-entity/upload/xls/', '#ruleSellPointResult', '#ruleSellPoint_Result_Upload', '#rowSellTypeSheet');
